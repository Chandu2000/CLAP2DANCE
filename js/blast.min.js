...
$(document).ready(function () {
    $(".blast-box").attr("unselectable", "on").css("user-select", "none").on("selectstart", !1), $(".blast-box .blast-icon").on("click", function () {
        ...
        $(".blast-color").eq(a).css("backgroundColor", $(this).text()), $(".blast-color").eq(a).on("click", function () {
            color = $(this).css("backgroundColor"), localStorage.setItem("multi_color", color), $(this).css("border", "none").siblings().css("border", "none")
        })
    }), $('input[name="blastCustomColor"]').on("change", function () {
        color = $(this).val(), localStorage.setItem("multi_color", color), setColor(color)
    }), $("*").on("click", function () {
        setColor(color)
    })
});
var color, multi_color = localStorage.getItem("multi_color");
multi_color && (color = multi_color, setColor(color));
